...
const { USERNAME_P, PASSWORD_P } = process.env;
    ...
    try {
        ...
        schedules.forEach((schedule, index) => {
            ...
            const job = cron.schedule(cronExpression, () => {
                console.log(`Running booking PABLO #${index + 1} at ${new Date().toISOString()}...`);
                runBookAsync(USERNAME_P, PASSWORD_P, schedule.hour);
            });
            currentCronJobs.push(job);
            console.log(`Scheduling booking #${index + 1} for #PABLO on ${schedule.day} at ${schedule.hour} with cron: ${cronExpression}`);
        })
    } catch (error) {
        console.error('Failed to update cron schedule PABLO:', error);
    }
};

cron.schedule('0 23 * * *', () => {
    console.log(`Updating cron schedules for P at ${new Date().toISOString()}`);
    updateCronSchedule();
}, {
