...
const { USERNAME_J, PASSWORD_J } = process.env;
    ...
    try {
        ...
        schedules.forEach((schedule, index) => {
            ...
            const job = cron.schedule(cronExpression, () => {
                console.log(`Running booking JORGE #${index + 1} at ${new Date().toISOString()}...`);
                runBookAsync(USERNAME_J, PASSWORD_J, schedule.hour);
            });
            currentCronJobs.push(job);
            console.log(`Scheduling booking #${index + 1} for #JORGE on ${schedule.day} at ${schedule.hour} with cron: ${cronExpression}`);
        })
    } catch (error) {
        console.error('Failed to update cron schedule JORGE:', error);
    }
};
